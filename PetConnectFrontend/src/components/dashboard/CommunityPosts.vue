...
type PostTypeFilter = "all" | "rehoming" | "breeder" | "adoption";
...
const SAVED_POST_STORAGE_PREFIX = "community-saved-post-ids";

const DEFAULT_PET_IMAGE = "https://placehold.co/300x220?text=No+Pet+Photo";
...
/*
 * keyword 搜索由后端 endpoint 完成。
 *
 * 前端这里只继续处理 Post Type filter，
 * 避免后端通过 species、breed 等字段匹配出的结果
 * 被前端再次错误过滤掉。
 */
const filteredPosts = computed(() => {
  return allPosts.value.filter((post) => {
    ...
    return post.type?.trim().toLowerCase() === selectedType.value;
  });
});

const totalPages = computed(() => {
  return Math.max(1, Math.ceil(filteredPosts.value.length / POSTS_PER_PAGE));
});
...
const visiblePageNumbers = computed(() => {
  ...
  const endPage = Math.min(total, startPage + maximumVisiblePages - 1);
...
const lastVisiblePage = computed(() => {
  return visiblePageNumbers.value[visiblePageNumbers.value.length - 1] ?? 1;
});
...
function loadSavedPostIds(): void {
  ...
  const storedValue = localStorage.getItem(getSavedPostStorageKey());
  ...
  try {
    ...
    const validPostIds = parsedValue.filter((value): value is number => {
      return typeof value === "number" && Number.isInteger(value) && value > 0;
    });

    savedPostIds.value = new Set(validPostIds);
  } catch {
    savedPostIds.value = new Set();
  }
}
...
function saveSavedPostIds(): void {
  ...
  localStorage.setItem(
    ...
    JSON.stringify([...savedPostIds.value]),
  );
}
...
function markPostAsSaved(postId: number): void {
  const updatedPostIds = new Set(savedPostIds.value);

  updatedPostIds.add(postId);

  savedPostIds.value = updatedPostIds;
...
async function readPostsResponse(
  ...
  try {
    return JSON.parse(responseText) as ApiResponse<Post[]>;
  } catch {
...
async function searchPosts(keyword: string): Promise<void> {
  ...
  try {
    ...
    const responseData = await readPostsResponse(response);
    ...
    allPosts.value = responseData.data ?? [];
...
function selectPostType(type: PostTypeFilter): void {
...
function formatLocation(post: Post): string {
  ...
  return [cityState, address.zipCode].filter(Boolean).join(" ");
}
...
async function fetchAllPosts(): Promise<void> {
  try {
    ...
    const response = await fetch("http://localhost:8080/post/get_all_post", {
      method: "GET",
      headers: {
        Accept: "application/json",
      },
...
async function refreshPosts(): Promise<void> {
  if (activeSearchKeyword.value) {
    await searchPosts(activeSearchKeyword.value);
...
async function viewPet(petId: number): Promise<void> {
  await router.push({
    name: "pet-detail",
    params: {
      petId: String(petId),
...
async function savePost(post: Post): Promise<void> {
  ...
  if (!userId) {
    errorMessage.value = "You must be logged in to save a post.";

    return;
  }

  if (savingPostId.value !== null || isPostSaved(post.postId)) {
    return;
  }
  ...
  try {
    ...
    let responseData: ApiResponse<unknown> | null = null;

    if (responseText) {
      try {
        responseData = JSON.parse(responseText) as ApiResponse<unknown>;
      } catch {
        responseData = null;
      }
    }
    ...
    if (responseData?.state === false) {
      throw new Error(responseData.message || "Unable to save post.");
    }

    markPostAsSaved(post.postId);
  } catch (error) {
    errorMessage.value =
  ...
  <div class="community-posts">
    <form class="search-box" @submit.prevent="handleSearch">
      <input
        ...
        type="search"
        placeholder="Search by title, content, type, pet ID, or location..."
      />
    ...
    <section class="filter-bar">
      <div class="filter-heading">
        <div>
          <p class="filter-label">Post Type</p>
        ...
        <button
          v-if="selectedType !== 'all' || searchKeyword"
          type="button"
      ...
      <div class="filter-options">
        <button
          ...
          @click="selectPostType(filter.value)"
        >
          {{ filter.label }}
          ...
          <span>
            {{
              filter.value === "all"
                ? allPosts.length
                : allPosts.filter(
                    (post) => post.type?.toLowerCase() === filter.value,
                  ).length
            }}
          </span>
        </button>
    ...
    <section class="welcome-card">
      <div>
        ...
        <h2>
          Welcome back,
          {{ props.user?.fullname || props.user?.username || "User" }}
        </h2>

        <p>Browse available pets and community adoption posts.</p>
      </div>
    </section>
    ...
    <section class="content-card">
        ...
        <div>
          ...
          <h2>
            {{
              selectedType === "all"
                ? "All Posts"
                : `${formatPostType(selectedType)} Posts`
            }}
          </h2>
        </div>
      ...
      <template v-else-if="paginatedPosts.length">
          ...
          >
            ...
            <div class="post-actions">
              ...
              <button
                ...
                class="save-button"
                :class="{
                  saved: isPostSaved(post.postId),
                }"
                :disabled="savingPostId !== null || isPostSaved(post.postId)"
                @click="savePost(post)"
              >
                {{
        ...
        >
          ...
          <span v-if="firstVisiblePage > 2" class="pagination-ellipsis">
            ...
          </span>
      ...
      <div v-else class="empty-state">
        ...
        <p v-if="searchKeyword || selectedType !== 'all'">
          No posts match the selected filters.
        </p>

        <p v-else>There are currently no community posts available.</p>
      </div>
    </section>
  </div>
</template>
...
@media (max-width: 440px) {
  ...
  .pagination-button:not(.pagination-navigation) {
    min-width: 36px;
    height: 36px;
    padding: 0 9px;
  }
